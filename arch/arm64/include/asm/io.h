...
{
	u8 val;
	asm volatile(ALTERNATIVE_NORELOC("ldrb %w0, [%1]",
					 "ldarb %w0, [%1]",
...
static __always_inline u16 __raw_readw(const volatile void __iomem *addr)
{
	...
	asm volatile(ALTERNATIVE_NORELOC("ldrh %w0, [%1]",
					 "ldarh %w0, [%1]",
					 ARM64_WORKAROUND_DEVICE_LOAD_ACQUIRE)
		     : "=r" (val) : "r" (addr));
	return val;
...
{
	u32 val;
	asm volatile(ALTERNATIVE_NORELOC("ldr %w0, [%1]",
					 "ldar %w0, [%1]",
					 ARM64_WORKAROUND_DEVICE_LOAD_ACQUIRE)
		     : "=r" (val) : "r" (addr));
	return val;
...
{
	u64 val;
	asm volatile(ALTERNATIVE_NORELOC("ldr %0, [%1]",
					 "ldar %0, [%1]",
					 ARM64_WORKAROUND_DEVICE_LOAD_ACQUIRE)
		     : "=r" (val) : "r" (addr));
	return val;
