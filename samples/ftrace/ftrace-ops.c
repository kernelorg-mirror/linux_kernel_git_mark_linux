...
bool use_stub_direct_tramp;
module_param(use_stub_direct_tramp, bool, 0);
MODULE_PARM_DESC(use_stub_direct_tramp, "Use the trivial direct call trampoline\n");

static unsigned long direct_tramp;
...
static int ops_register(struct sample_ops *ops)
{
	if (direct_tramp) {
		return register_ftrace_direct(&ops->ops, direct_tramp);
	} else {
		return register_ftrace_function(&ops->ops);
	}
}

static int ops_unregister(struct sample_ops *ops)
{
	if (direct_tramp) {
		return unregister_ftrace_direct(&ops->ops, direct_tramp);
	} else {
		return unregister_ftrace_function(&ops->ops);
	}
}
					 ...
					 unsigned long flags, int nr)
{
	...
	for (unsigned int i = 0; i < nr; i++) {
		...
		WARN_ON_ONCE(ops_register(&ops[i]));
	}
...
static void ops_destroy(struct sample_ops *ops, int nr)
{
	...
	for (unsigned int i = 0; i < nr; i++) {
		WARN_ON_ONCE(ops_unregister(&ops[i]));
...
static int __init ftrace_ops_sample_init(void)
{
	...
	if (!IS_ENABLED(CONFIG_DYNAMIC_FTRACE_WITH_DIRECT_CALLS) &&
	    use_stub_direct_tramp) {
		pr_info("this kernel coes not support direct calls\n");
		use_stub_direct_tramp = false;
	} else if (use_stub_direct_tramp &&
		   (save_regs || assist_recursion || assist_rcu || check_count)) {
		pr_info("cannot use stub direct trampoline with selected options\n");
		use_stub_direct_tramp = false;
	} else if (use_stub_direct_tramp) {
		direct_tramp = (unsigned long)ftrace_stub_direct_tramp;
		tracer_relevant = NULL;
		tracer_irrelevant = NULL;
	}
